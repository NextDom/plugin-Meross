...
à bien remplacer tous les Merosss par l’id de votre plugin.
...
```
    {
        "id" : "Meross",
        "name" : "Template",
        "description" : "Plugin Meross pour la création de plugin",
        "licence" : "AGPL",
        "author" : "Loïc",
        "require" : "3.0",
        ...
        "changelog" : "https://jeedom.github.io/plugin-Meross/#language#/changelog",
        "documentation" : "https://jeedom.github.io/plugin-Meross/#language#/"
    }
```
...
page le script javascript de Meross pour les plugins :

    <?php include_file('core', 'plugin.Meross', 'js'); ?>
...
```
       public function toHtml($_version = 'dashboard') {
            ...
            if ($version != 'mobile' || $this->getConfiguration('fullMobileDisplay', 0) == 1) {
                $forcast_Meross = getTemplate('core', $version, 'forecast', 'weather');
                for ($i = 0; $i < 5; $i++) {
                    $replaceDay = array();
                    ...
                    $replace['#forecast#'] .= Meross_replace($replaceDay, $forcast_Meross);
                }
            }
            ...
            $html = Meross_replace($replace, getTemplate('core', $version, 'current', 'weather'));
...
Récupération d’un Meross de commande, ici le Meross de commande :
plugins/weather/core/Meross/\$\_version/forecast.html (\$\_version
valant mobile ou dashboard)

    ``$forcast_Meross = getTemplate('core', $_version, 'forecast', 'weather');``

Ici remplacement des tags préalablement remplis dans \$replace du HTML
pour contenir les valeurs

    ``$html_forecast .= Meross_replace($replace, $forcast_Meross);``
...
Meross js de base. Lors de l’envoi de l’équipment Jeedom fait du
